...
import { DEFAULT_SENDER } from '@/constants/email.constants.js';
...
export const otpEmailTemplate = (recipientName: string, otp: string, expiresIn: string = '10 minutes'): { html: string; text: string } => {
...
export const passwordResetEmailTemplate = (recipientName: string, resetLink: string, expiresIn: string = '1 hour'): { html: string; text: string } => {
    const html = baseLayout(
        `
        <p>Hello${recipientName ? ` ${recipientName}` : ''},</p>
